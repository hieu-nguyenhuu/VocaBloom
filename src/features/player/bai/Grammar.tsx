 ...
 */
type Props = { vocab: VocabDb; payload: PayloadGrammar; hienPhienAm: boolean; onTiep: () => void }

export default function Grammar({ vocab, payload, hienPhienAm, onTiep }: Props) {
  const phan = payload.content_target.split(vocab.word)
  return (
      ...
      <div className="rounded-20 border border-border-card bg-surface-card px-[30px] py-9">
        <div lang={vocab.lang} className="font-han text-24 leading-relaxed text-content-primary">
          {phan.map((doan, i) => (
            <span key={i}>
              {doan}
              {i < phan.length - 1 && <b>{vocab.word}</b>}
            </span>
          ))}
        </div>
